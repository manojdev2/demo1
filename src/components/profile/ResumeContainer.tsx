...
function ResumeContainer({ resume }: { resume: Resume }) {
  ...
  // Find sections with experience or education data
  // Due to unique constraint, they may share the same section
  const experienceSection = ResumeSections?.find(
    (section) => 
      section.workExperiences && 
      section.workExperiences.length > 0 &&
      section.sectionType !== SectionType.SUMMARY
  );
