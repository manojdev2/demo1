...
import { AppError, DatabaseError, ExternalServiceError } from "./errors";
 ...
 * Wraps errors with context and proper error types.
 *
...
export function handleError(
  ...
  if (error instanceof Error) {
    ...
    if (error.message === "fetch failed" || error.message.includes("fetch")) {
      wrappedError = new ExternalServiceError(
      ...
      wrappedError = new AppError(error.message || msg, {
        context,
        originalError: error,
      });
    }
...
export const getLast7Days = (dateType = "PP"): string[] => {
  const dates = [];
  for (let i = 6; i >= 0; i--) {
    const date = new Date();
